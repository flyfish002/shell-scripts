...
#check shanghai vpn sh_hub_1 gateway  send some ping      
  pingreceivecount=`ping -c $ping_count $sh_hub_1_gw_ip | grep loss | cut -d \, -f 2 | awk '{print $1}'`  
  #echo  “cur ping receive count：”   $pingreceivecount
  if [ $pingreceivecount="0" ];then
